...
class Interactor implements PostInterface
{
    ...
    private function registerAdminRoutes()
    {
        ...
        foreach ($postTypes as $postType => $postTypeArgs) {
            \Route::resource($postTypeArgs['route'], '\Plugins\PostType\Http\PostController', [
              'names' => [
            ...
            \Route::get($postTypeArgs['slug'].'/{id}/restore', ['as' => 'admin.'.$postTypeArgs['slug'].'.restore', 'uses' =>'\Plugins\PostType\Http\PostController@restore']);
            \Route::get($postTypeArgs['slug'].'/{id}/trash', ['as' => 'admin.'.$postTypeArgs['slug'].'.trash', 'uses' =>'\Plugins\PostType\Http\PostController@trash']);
            ...
            foreach ($registeredTaxonomies as $taxonomy => $taxonomyArgs) {
                if (in_array($postType, $taxonomyArgs['post_type'])) {
                    ...
                    \Route::get($postTypeArgs['route'].'/'.$taxonomy.'/index', ['as' => 'admin.'.$postTypeArgs['route'].'.'.$taxonomy.'.index', 'uses' =>'\Plugins\PostType\Http\TaxonomyController@index']);
                    \Route::get($postTypeArgs['route'].'/'.$taxonomy.'/create', ['as' => 'admin.'.$postTypeArgs['route'].'.'.$taxonomy.'.create', 'uses' =>'\Plugins\PostType\Http\TaxonomyController@create']);
                    \Route::post($postTypeArgs['route'].'/'.$taxonomy.'/store', ['as' => 'admin.'.$postTypeArgs['route'].'.'.$taxonomy.'.store', 'uses' =>'\Plugins\PostType\Http\TaxonomyController@store']);
                    \Route::get($postTypeArgs['route'].'/'.$taxonomy.'/{id}/edit', ['as' => 'admin.'.$postTypeArgs['route'].'.'.$taxonomy.'.edit', 'uses' =>'\Plugins\PostType\Http\TaxonomyController@edit']);
                    \Route::post($postTypeArgs['route'].'/'.$taxonomy.'/{id}/update', ['as' => 'admin.'.$postTypeArgs['route'].'.'.$taxonomy.'.update', 'uses' =>'\Plugins\PostType\Http\TaxonomyController@update']);
                    \Route::get($postTypeArgs['route'].'/'.$taxonomy.'/{id}/destroy', ['as' => 'admin.'.$postTypeArgs['route'].'.'.$taxonomy.'.destroy', 'uses' =>'\Plugins\PostType\Http\TaxonomyController@destroy']);
                }
            }
        }
    }
    ...
    public function registerPostType($postType, $args)
    {
        ...
        $menu->addSubMenuPage('admin.'.$args['route'].'.index', __('post-type::default.all-post-type', ['post-type' => $args['label']['name']]), __('post-type::default.all-post-type', ['post-type' => $args['label']['name']]), 'admin.'.$args['route'].'.index');
