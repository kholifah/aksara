          ...
          @foreach($modules as $module)
            <tr>
              ...
              <td>
                @if($module->getActive())
                  <form method='POST' action="{{ route('module-manager.deactivate',['slug'=>$module->getName(),'type'=>$module->getType()]) }}">
                    {{ csrf_field() }}
                    <input type='submit' class='btn btn-xs btn-default' value="{{  __('core:module-manager::default.deactivate') }}" {{ can_disable_module($module->getType(), $module->getName()) ? ''  : 'disabled' }}>
                  </form>
                @else
              ...
              <td>
                ...
                @endif
                @if (is_plugin_required($module->getName()))
                  {{  __('core:module-manager::default.currently-used-by') }}:
                  @foreach (get_plugin_required_by($module->getName()) as $requiredByItem)
                    {{ $requiredByItem }}
                  @endforeach
                @endif
              </td>
        ...
        @foreach( \Config::get('aksara.modules.plugin',[])  as $moduleName => $moduleDescription )
          <tr>
            ...
            <td>
              ...
              @if($moduleV1->getModuleStatus('plugin',$moduleName))
                <form method='POST' action="{{ route('module-manager.deactivate',['slug'=>$moduleName,'type'=>'plugin']) }}">
                  {{ csrf_field() }}
                  <input type='submit' class='btn btn-xs btn-default' value="{{  __('core:module-manager::default.deactivate') }}" {{ can_disable_module('plugin', $moduleName) ? '' : 'disabled' }}>
                </form>
              @else
            ...
            <td>
              ...
              @endif
              @if (is_plugin_required($moduleName))
                {{  __('core:module-manager::default.currently-used-by') }}:
                @foreach (get_plugin_required_by($moduleName) as $requiredByItem)
                  {{ $requiredByItem }}
                @endforeach
              @endif
            </td>
